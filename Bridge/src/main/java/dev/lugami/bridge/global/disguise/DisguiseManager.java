...
import dev.lugami.bridge.global.packet.types.DisguisePacket;
...
//				Msg.logConsole(" - Signature: " + fakeSkin.getProperty().getSignature());
	...
	public void undisguise(Player player, boolean full, boolean quit) {
		...
		if(disguisePlayer != null) {
			...
			if(!quit) {
				MinecraftServer.getServer().getPlayerList().playersByName.put(profile.getUsername(), ((CraftPlayer) player).getHandle());
//				Bukkit.broadcastMessage("Putting " + profile.getUsername());
			}

			MinecraftServer.getServer().getPlayerList().playersByName.remove(player.getName());
