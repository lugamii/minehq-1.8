...
public class TabListManager implements Listener {
    ...
    public TabListManager() {
        qLib.getInstance().getServer().getPluginManager().registerEvents(this, qLib.getInstance());
        new TabRunnable().runTaskTimerAsynchronously(qLib.getInstance(), 2L, 20L);
    }
    ...
    public void updateSlot(UUID uuid, int row, int column, String value) {
        if (players.contains(uuid)) {
            Player player = qLib.getInstance().getServer().getPlayer(uuid);
            if (player != null) {
                tabList.updateSlot(player, row, column, value);
            }
        }
