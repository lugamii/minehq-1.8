...
import dev.lugami.qlib.packet.ScoreboardTeamPacketMod;
...
final class FrozenScoreboard {
    ...
    private final ThreadLocal<dev.lugami.qlib.util.LinkedList<String>> localList = ThreadLocal.withInitial(dev.lugami.qlib.util.LinkedList::new);


    public FrozenScoreboard(Player player) {
        this.player = player;

        final org.bukkit.scoreboard.Scoreboard board = qLib.getInstance().getServer().getScoreboardManager().getNewScoreboard();

        this.objective = board.registerNewObjective("qLib", "dummy");
    ...
    public void update() {
        ...
        final String untranslatedTitle = FrozenScoreboardHandler.getConfiguration().getTitleGetter().getTitle(this.player);
