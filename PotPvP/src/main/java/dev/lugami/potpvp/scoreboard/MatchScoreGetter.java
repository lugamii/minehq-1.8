...
import dev.lugami.potpvp.kittype.HealingMethod;
...
import dev.lugami.potpvp.pvpclasses.pvpclasses.BardClass;
import dev.lugami.qlib.autoreboot.AutoRebootHandler;
import dev.lugami.qlib.scoreboard.ScoreFunction;
...
public class MatchScoreGetter implements BiConsumer<Player, dev.lugami.qlib.util.LinkedList<String>> {
    ...
    @Override
    public void accept(Player player, dev.lugami.qlib.util.LinkedList<String> scores) {
        ...
        if (match == null) {
            if (followingOpt.isPresent()) {
                scores.add("&bFollowing: *&f" + UUIDUtils.name(followingOpt.get()));
            }
...
//        int calculatedSecond = theirHits - yourHits;
        ...
        scores.add("&bOpponent: &f" + other.getName());
...
//        }
        ...
        if (teams.size() == 2) {
            ...
            if (teamOne.getAllMembers().size() != 1 && teamTwo.getAllMembers().size() != 1) {
                ...
                if (match.getKitType().getId().equals("Boxing")) {
                    ...
                    scores.add(" &b" + FrozenUUIDCache.name(teamTwo.getFirstAliveMember()) + ": &f" + theirHits);
                }
            }
        }
    }
    ...
    private void renderMetaLines(List<String> scores, Match match, boolean participant) {
        ...
        scores.add(PotPvPSI.getInstance().getDominantColor() + "Duration: &f" + formattedDuration);
    }
    ...
    private void renderBoxingLines(List<String> scores, MatchTeam otherTeam, Player player, Match match) {
        ...
        if (match.getKitType().getId().equals("Boxing") && !match.isSpectator(player.getUniqueId())) {
            ...
            scores.add("&bHits: " + ((yourHits >= theirHits ? ChatColor.GREEN + "(+" + calculatedFirst + ")" : ChatColor.RED + "(-" + calculatedSecond + ")")));
            scores.add(" &aYou: &f" + yourHits + (yourCombos >= 1 ? ChatColor.YELLOW + " (" + yourCombos + " Combo)" : ""));
            scores.add(" &cThem: &f" + theirHits + (theirCombos >= 1 ? ChatColor.YELLOW + " (" + theirCombos + " Combo)" : ""));
        }
    }
